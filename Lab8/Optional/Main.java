...
public class Main {
    public static void main(String[] args) {
        ...
        for(int i=0,j=rand.nextInt(artists_number);i<albums_number;i++,j=rand.nextInt(artists_number))
        {
            ...
            Album tmp;
            do {
                tmp=new Album(faker.idNumber().valid(),artistList.get(j).getId(),faker.book().title(),1950+rand.nextInt(70),rand.nextInt(1000000));
                ok=tmp.insertDB();
            }while(!ok);
            albumList.add(tmp);
        }
        for(int i=0;i<charts_number;i++)
        {
            ...
            {
                while(!ChartPositions.insertDB(albumList.get(rand.nextInt(albums_number)).getId(),chartList.get(i).getId(),rand.nextInt(number_of_albums_in_chart)+1));
            }
        }
        try {
            Statement stmt = Database.getConnection().createStatement();
